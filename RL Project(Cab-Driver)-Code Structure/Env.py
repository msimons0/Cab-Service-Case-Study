...
class CabDriver():
    ...
    """convert the state into a vector so that it can be fed to the NN. This 
    method converts a given state into a vector format. Hint: The vector is of 
    size m + t + d."""
    ...
    # """convert the (state-action) into a vector so that it can be fed to the.
    # NN. This method converts a given state-action pair into a vector format. 
    ...
    def requests(self, state):
        ...
        possible_actions_index = random.sample(range(1, (m-1)*m ), requests)
    ...
    def reward_func(self, state, action, Time_matrix):
        ...
        time_of_day = state[1]
        day_of_week = state[2]
        #variable to calculate the time when cab reached pickup posiion if curr_pos != pickup_pos
        new_time_of_day = time_of_day
        #variable to calculate the day of week when cab reached pickup posiion if curr_pos != pickup_pos
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        time_of_day = state[1]
        day_of_week = state[2]
        # Calculate the time when cab reaches the pickup position if curr_pos != pickup_pos
        new_time_of_day = time_of_day
        # Calculate the day of week when cab reaches the pickup position if curr_pos != pickup_pos
        new_day_of_week = day_of_week
        total_time = 0
        if cab_pos!=pickup_pos:
            # Calculate the new time of day and week when cab reaches pickup position
            time_taken = Time_matrix[cab_pos][pickup_pos][time_of_day][day_of_week]
            new_time_of_day, new_day_of_week = self.new_time(time_of_day, day_of_week, time_taken)
